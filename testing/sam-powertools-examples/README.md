...
```bash
sam-powertools-example$ sam build
```
...
```bash
sam-powertools-example$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-powertools-example$ sam local start-api
sam-powertools-example$ curl http://localhost:3000/
```
...
```bash
sam-powertools-example$ sam logs -n HelloWorldFunction --stack-name sam-powertools-example --tail
```
...
```bash
sam-powertools-example$ cd layer
layer$ npm install
layer$ npm run test
```
...
```bash
sam delete --stack-name sam-powertools-example
```
